...
"""
Creates a SE-ResNet Model as defined in:
...
def se_resnet(bottleneck=True, baseWidth=64, head7x7=True, layers=(3, 4, 23, 3), num_classes=1000):
    """
    Construct SE_ResNet.
    (2, 2, 2, 2) for se_resnet18	# bottleneck=False
    (2, 2, 2, 2) for se_resnet26
    (3, 4, 6, 3) for se_resnet34	# bottleneck=False
    (3, 4, 6, 3) for se_resnet50
    (3, 4, 23, 3) for se_resnet101
    (3, 8, 36, 3) for se_resnet152
    note: if you use head7x7=False, the actual depth of se_resnet will increase by 2 layers.
    """
    model = SE_ResNet(bottleneck=bottleneck, baseWidth=baseWidth, head7x7=head7x7, layers=layers,
