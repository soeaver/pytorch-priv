...
""" 
Squeeze-and-Excitation Aligned Inception ResNeXt
...
class SEAIRXBottleneck(nn.Module):
    """
    SEAIRXBottleneck bottleneck type C
    """
    expansion = 4
    ...
    def __init__(self, inplanes, planes, baseWidth, cardinality, stride=1, downsample=None):
        ...
        """
        super(SEAIRXBottleneck, self).__init__()
...
def se_airx(baseWidth=64, head7x7=True, layers=(3, 4, 23, 3), num_classes=1000):
    """
    Construct SE_AIRX.
    (2, 2, 2, 2) for se_airx26
    (3, 4, 6, 3) for se_airx50
    (3, 4, 23, 3) for se_airx101
    (3, 8, 36, 3) for se_airx152
    note: The actual depth of se_airx is not the same as its name.
    """
    model = SE_AIRX(baseWidth=baseWidth, head7x7=head7x7, layers=layers, num_classes=num_classes)
    return model
